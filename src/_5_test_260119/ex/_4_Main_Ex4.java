...
public class _4_Main_Ex4 {
    static void main(String[] args) {
        // 최대 5명까지 저장 가능한 배열 생성
        _4_MemberBase_Ex4[] members = new _4_MemberBase_Ex4[5];
        ...
        while (true) {
            ...
            try {
                // 콘솔에 입력된 내용들 타입: 문자열
                // 그래서, 입력 받은 문자열을 숫자 형태로 변경
                // Integer.parseInt: 문자열 -> 숫자로 타입 변경 해주는 기능
                choice = Integer.parseInt(sc.nextLine());
            } catch (NumberFormatException e) {
                System.out.println("숫자만 입력해주세요.");
                // 콘솔에 숫자가 아닌 다른 문자등을 입력시에는, 반복문이 종료되지 않고 계속 동작 하게끔 설정
                continue;
            }
            switch (choice) {
                case 1: // 회원 가입
                    ...
                    // 260120_실습4_풀이, 순서8, 생성자가 기존 매개변수3개에서 4개로 수정
//                    _3_NormalMember newMember = new _3_NormalMember(name, email, age);
                    _4_NormalMember_Ex4 newMember = new _4_NormalMember_Ex4 (name, email, age, password);
                    // 배열에 저장
                ...
                case 3:
                    // 순서대로 확인
                    System.out.println("\n====로그인===== ");
                    // 이메일, 패스워드 정보를 전달 받아서 배열에 등록된 이메일로 유저를 찾고, 패스워도 비교하고
                    // 일치하면, 로그인, 아니면 로그인 불가.
                    System.out.println("이메일 : ");
